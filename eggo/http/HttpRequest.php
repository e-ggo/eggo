...
class HttpRequest
{
    ...
    public static function Single(Request $request, Response $response)
    {
        if (null === self::$instance) {
            self::$instance = new static($request, $response);
        }
    ...
    public function validMethod(string $data, string $method): bool
    {
        ...
        } else $valid = $this->checkMethods($data, $method);
    ...
    public function getMethod(): string
    {
        ...
        if (!empty($formMethod)) $method = strtoupper($formMethod);
        return $method;
    }
    ...
    protected function checkMethods(string $value, string $method): bool
    {
        if (in_array($value, explode('|', $this->validMethods))) {
            if ($this->request->isXmlHttpRequest() && $value === 'AJAX') return true;

            if ($this->request->isXmlHttpRequest() && strpos($value, 'X') === 0
                && $method === ltrim($value, 'X')) return true;
            if (in_array($value, [$method, 'ANY'])) return true;
        }
        return false;
    }
